  ...
  useEffect(() => {
    if (questionnaire) {
      form.reset({
        ...
        fundingToken: questionnaire.fundingToken || "PULSE",
      })
...
function SortablePollItem({ poll, index, onRemove, disabled }: SortablePollItemProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    >
      <CardContent className="p-3 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-medium text-sm">Poll #{poll.pollId}</p>
